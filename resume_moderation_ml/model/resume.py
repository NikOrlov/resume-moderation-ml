...
_ALL_RESUME_FIELDS = [
    'accessType', 'additionalEducation', 'advancedKeySkills', 'age', 'area', 'attestationEducation', 'birthday',
    'blacklist', 'businessTripReadiness', 'certificate', 'citizenship', 'creationTime', 'educationLevel',
    'elementaryEducation', 'email', 'employment', 'firstName', 'gender', 'id', 'keySkills', 'lang', 'language',
    'lastChangeTime', 'lastChangeTimeDetails', 'lastExperience', 'lastName', 'markService', 'metro', 'metroStation',
    'middleName', 'moderFullName', 'moderationFlags', 'moderationNotes', 'moderationTime', 'personalSite', 'phone',
    'photo', 'photoUrls', 'portfolio', 'portfolioUrls', 'preferredContact',
    'primaryEducation', 'recommendation', 'relocation', 'relocationArea', 'renewalService', 'salary',
    'secretWord', 'skills', 'source', 'status', 'title', 'totalExperience', 'travelTime', 'userId',
    'vacancyTypesForResponse', 'whitelist', 'workSchedule', 'workTicket'
]

_NON_INFORMATIONAL_FIELDS = [
    'accessType', 'blacklist', 'id', 'lang', 'lastChangeTime', 'lastChangeTimeDetails', 'lastExperience',
    'markService', 'metro', 'moderFullName', 'moderationFlags', 'moderationNotes', 'moderationTime', 'photoUrls',
    'portfolioUrls', 'relocation', 'renewalService', 'secretWord', 'source', 'status', 'userId',
    'vacancyTypesForResponse', 'whitelist'
]
...
def extract_phone_number(phone_json):
    return ''.join(re.sub(r'[^0-9]*', '', phone_json[part])
...
def _make_index_exp_query(index=None):
    if index is not None:
        return {
            'experience': {
                index: ['companyName', 'description', 'position']
            }
        }
    else:
...
class FieldSizeQuotients(object):
    ...
    def __call__(self, resume):
        ...
        if resume['experience']:
            ...
            exp_len_percentiles = np.concatenate([np.array([np.percentile(exp_lens, p) for p in _EXP_LEN_PERCENTILES]),
                                                  np.array([exp_lens.mean(), exp_lens.std()])])

        stats = [np.array([last_exp_len,
                           last_exp_len / (1.0 + full_exp_len),
                           last_exp_len / (1.0 + full_text_len),
                           full_exp_len / (1.0 + full_text_len),
                           full_edu_len / (1.0 + full_text_len)])]
        stats.extend([exp_len_percentiles,
...
class ResumeNumberStats(object):
    def __init__(self):
        ...
        for prefix in ('num', 'phone'):
            feature_names.extend([
                '{}_{}'.format(prefix, feature_name) for feature_name in
                ('long_count', 'long_78_count', 'count', 'len_mean', 'len_std')
            ])
    ...
    def __call__(self, text):
        return np.concatenate([
            ...
            get_number_sequences_stats(list(filter(None, [u''.join(_NUM_PATTERN.findall(item))
                                                          for item in _PHONE_NUMBER_SUSPICION.findall(text)])))
        ])
...
class ManualRejectRules(object):
    def __call__(self, resume):
        ...
        if ((resume['age'] is None or resume['age'] >= 27) and
                not resume['experience']):
            return 1.0
