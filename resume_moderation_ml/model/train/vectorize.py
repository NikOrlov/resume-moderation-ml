...
from resume_moderation_ml.model.resume import (
    POSSIBLE_PHONE_FIELDS_EXTRACTOR, FieldSizeQuotients, ManualRejectRules, ResumeNullFieldsCount, ResumeNumberStats,
    SalaryExtractor, StartOfCareerConsistency, get_age, get_age_experience_difference, get_gender,
    get_industry_fullness, get_known_university_count, get_longest_phone_char, get_time_without_education,
...
def get_vectorizer_pipe():
    ...
    for field in text_fields.FIELDS_TO_ANALYZE:
        ...
        feature_names.extend([u'{}__{}'.format(field, feature_name)
    ...
    full_text_pipe = Pipeline([
        ...
        ('union', FeatureUnion([
            ('token_features', Pipeline([
                ...
                ('union', FeatureUnion([
                    ('statistics', TokenLengthStatistics(percentiles=np.linspace(0.0, 100.0, 11))),
                    ('word_count', InputLength())
                ]))
            ])),
    ...
    feature_names.extend([
        u'full_text__{}'.format(feature_name) for feature_name in
        full_text_pipe.named_steps['union'].transformer_list[0][1].named_steps['union'].get_feature_names()
    ])
    ...
    transformers.append(('number_sequences_stats', Pipeline([
        ('extract', POSSIBLE_PHONE_FIELDS_EXTRACTOR),
        ('stats', ValueExtractor(resume_number_stats))
    ])))
    ...
    transformers.append(('salary', Pipeline([('salary', ValueExtractor(SalaryExtractor(load_currency_rates()))),
                                             ('log', LogTransformer())])))
    feature_names.append('log_salary')

    transformers.append(('total_experience', Pipeline([
        ('total_experience', ValueExtractor(get_total_experience)),
        ('log', LogTransformer())
    ])))
    ...
    feature_names.extend(['{}_len'.format(f) for f in (
        'experience', 'primaryEducation', 'additionalEducation', 'attestationEducation', 'keySkills',
        'firstName', 'middleName', 'lastName')])

    # start of career consistency
    start_of_career_consistency = StartOfCareerConsistency()
    transformers.append((
        'start_of_career_consistency',
        ValueExtractor(start_of_career_consistency)
    ))
