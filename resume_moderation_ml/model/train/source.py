...
from resume_moderation_ml.model.train import cache_obj, iterate_file_lines
...
def iterate_raw_source_csv(input_file):
    ...
    for row in reader:
        ...
        if status != "deleted" and creation_time < _CREATION_TIME_THRESHOLD:
            logger.debug(
                "skip record with id %s, creation time is before threshold: %s",
                id_,
...
def _extract_raw_data(dataset="main"):
    ...
    for id_, status, source_version, moderated_version, complete_status, creation_time in itertools.chain(
        iterate_raw_source_csv(csv_lines_iterator()), iterate_raw_source_csv(get_source_csv_lines_from_db())
    ):
    ...
    logger.info(
        "created %d data objects eligible for learning, %s",
        targets.size,
...
def split_main_vectorizer():
    if os.path.exists(_SOURCE_RESUME_CSV_FILES_FROM_HIVE["main"]) and os.path.exists(
        _SOURCE_RESUME_CSV_FILES_FROM_HIVE["vectorizer"]
    ):
        return
    ...
    main_resume_ids, vectorizer_resume_ids = map(
        set,
        train_test_split(
            allowed_ids,
            test_size=resume_moderation_config.vectorizer_size,
            random_state=resume_moderation_config.cv_seed,
        ),
    )

    with open(_SOURCE_RESUME_CSV_FILES_FROM_HIVE["all"], "r") as in_file, open(
        _SOURCE_RESUME_CSV_FILES_FROM_HIVE["main"], "w"
    ) as main_file, open(_SOURCE_RESUME_CSV_FILES_FROM_HIVE["vectorizer"], "w") as vectorizer_file:
        for line in in_file:
            try:
